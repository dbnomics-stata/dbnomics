...
Stata client for DB.nomics, the world's economic database (https://db.nomics.world)
    ...
    ##Syntax
			...
			```
            dbnomics datastructure , provider(PRcode) dataset(DScode) [clear
            nostat]
			```

            Load list of series for a given provider and dataset
			```
            dbnomics series , provider(PRcode) dataset(DScode) [clear limit(int)
            dimensions_opt]
			```

            Import series for a given provider and dataset
			```
            dbnomics import , provider(PRcode) dataset(DScode) [clear limit(int)
            seriesids(SERIES_list) sdmx(SDMX_mask) dimensions_opt]
			```

            Load one series via direct reference
			```
            dbnomics use SERIES_code , provider(PRcode) dataset(DScode) [clear
            delimiter(char)]
			```
        ...
        -------------------------------------------------------------------------
        Main
          provider(PRcode)         declare the reference data provider (e.g.
                                     AMECO, IMF, Eurostat)
          dataset(DScode)          declare the reference dataset (e.g. PIGOT,
                                     BOP, ei_bsin_m)
          clear                    replace data in memory

        For datastructure only
          nostat                   disable the parsing of series statistics
                                     (speeds up importing)

        For series and import only
          limit(int)               set the limit for the number of series to load
          dimensions_opt           provider- and dataset-specific options to
                                     filter series (see Examples)

        For import only
          seriesids(SERIES_list)   input a comma-separated list to load specific
                                     time series
          sdmx(SDMX_mask)          input an SDMX mask to select specific series
    ...
    ##Examples
        ...
        Load the dataset tree of of the AMECO provider:
            . dbnomics tree, provider(AMECO) clear    ///  (click to run)
    ...
    > (click to run)
        ...
        List all series in AMECO/PIGOT containing deflators in national currency:
            . dbnomics series, provider(AMECO) dataset(PIGOT) unit(national-currenc
    > y-2010-100) clear
            (click to run)
            40 of 86 series selected. Order of dimensions: (freq.unit.geo)

        Import all series in AMECO/PIGOT containing deflators in national
        currency:
            . dbnomics import, provider(AMECO) dataset(PIGOT) unit(national-currenc
    > y-2010-100) clear
            (click to run)
            ........................................
        ...
        Import single AMECO/PIGOT series:
            . dbnomics import, pr(AMECO) d(PIGOT) seriesids(ESP.3.1.0.0.PIGOT,SVN.3
    > .1.0.0.PIGOT,LVA.3.1.99.0.PIGOT) clear
            (click to run)
            ...
        ...
        Do the same using sdmx:
            . dbnomics import, provider(Eurostat) dataset(ei_bsin_m) sdmx(M.BAL...B
    > E) clear
            (click to run)
            ................
            16 series found and imported
